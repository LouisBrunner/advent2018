...
{
    changes
        .map(|change| -> Result<i32, core::Error> {
            if change.len() < 1 {
                return Err(core::Error::Internal {
                    why: format!("empty frequency change"),
                });
            }
            ...
            if change.starts_with('+') {
                ...
                Err(core::Error::Internal {
                    why: format!("invalid frequency change: `{}`", change),
                })
            }
        })
